...
export default function QuestionAnswersPage() {
  const { questionAnswers, searchTerm, setSearchTerm, setIsModalOpen, setModalMode, setSelectedItem, handleDelete } = useContext(DataContext);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-800 mb-2">Управление ответами на вопросы</h1>
          <p className="text-gray-600">Модерация и управление ответами</p>
        </div>
        <button
      ...
      <div className="grid gap-6">
        {questionAnswers
          .filter(answer => answer.textOriginal.toLowerCase().includes(searchTerm.toLowerCase()))
          .map(answer => (
            <div key={answer.id} className="bg-white rounded-2xl shadow-md border border-gray-100 hover:shadow-lg transition-shadow p-6">
              <div className="flex flex-col lg:flex-row gap-6">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-3">
                    <span
                      className={`px-3 py-1 rounded-full text-sm font-medium ${
                        answer.isModerated ? 'bg-green-50 text-green-600' : 'bg-yellow-50 text-yellow-600'
                      }`}
                    >
                  ...
                  <p className="text-gray-600 mb-4 line-clamp-3">{answer.textOriginal}</p>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={() => handleDelete('questionAnswer', answer.id)}

...
export default function SatelliteKeyModal({ isOpen, onClose, mode, satelliteKey, onSubmitSuccess }: SatelliteKeyModalProps) {
  const [formData, setFormData] = useState({
    keyUk: satelliteKey?.keyUk || '',
    keyRu: satelliteKey?.keyRu || '',
    slugUk: satelliteKey?.slugUk || '',
    slugRu: satelliteKey?.slugRu || '',
    metaTitleUk: satelliteKey?.metaTitleUk || '',
    metaTitleRu: satelliteKey?.metaTitleRu || '',
    metaDescUk: satelliteKey?.metaDescUk || '',
    metaDescRu: satelliteKey?.metaDescRu || '',
    titleUk: satelliteKey?.titleUk || '',
    titleRu: satelliteKey?.titleRu || '',
    descriptionUk: satelliteKey?.descriptionUk || '',
    descriptionRu: satelliteKey?.descriptionRu || '',
  });

  useEffect(() => {
    if (mode === 'edit' && satelliteKey) {
      setFormData({
        keyUk: satelliteKey.keyUk || '',
        keyRu: satelliteKey.keyRu || '',
        slugUk: satelliteKey.slugUk || '',
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (mode === 'create') {
        // await SatelliteKeyService.createSatelliteKey(payload);
        toast.success("Ключ сателлита успешно создан");
      } else {
        // await SatelliteKeyService.updateSatelliteKey(satelliteKey?.id, payload);
        toast.success("Ключ сателлита успешно обновлён");
      }

      onSubmitSuccess();
      onClose();
    } catch (error) {
      console.error("Ошибка при сохранении ключа сателлита:", error);
      toast.error("Ошибка при сохранении ключа сателлита");
    }
  };
      ...
      <div className="bg-white rounded-2xl shadow-xl max-w-4xl w-full max-h-[90vh] scrollbar-none overflow-y-auto">
        ...
        <form onSubmit={onSubmit} className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700">Ключ (укр.)</label>
              <input
                name="keyUk"
                type="text"
                value={formData.keyUk}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700">Ключ (рус.)</label>
              <input
                name="keyRu"
                type="text"
                value={formData.keyRu}
                onChange={handleChange}
                className="mt-1 w-full px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500"
                required
              />
